...
You need a file with information about users and passwords in order to run Miril. The location of the users file is specified in the C<user_config> option in the main configuration file. Currently only XML is supported as the configuration file format. The structure is as follows:
...
Additionally, each template, whether for a content type or a list, is passed a C<cfg> parameter, which is a copy of the Miril configuration object. The actual structure of this object depends on the what you have in your configuration file.
...
I will probably set up a group at Google Groups soon. In the meanwhile, contact me directly by email for any questions or help.
